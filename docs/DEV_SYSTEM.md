...
> (strategy); this file = **how** (conventions + setup). Operational status — the suite root's
> [`SUITE_STATUS.md`](../SUITE_STATUS.md) (snapshot auto-generated by `scripts/suite-status.sh`,
> weekly targets rewritten by the `/weekly` ritual) — not here.
...
- **In the suite root:** `README.md`, `VISION.md`, and the shared **`CLAUDE.md`**; supporting docs
  ...
  (local-only). (Operational status: suite-root `SUITE_STATUS.md` — see the header.)
